...
class ObservableCardReader : virtual public CardReader {
 public:
  ...
  enum NotificationMode {
    ...
    ALWAYS,
    /**
     * Only the cards that have been successfully selected will be notified to
     * the observers. The others will be ignored and the application will not be
   ...
   *
   * <p>The invocation of this method is <b>mandatory</b> when the reader has to
   * be observed.
   *
   ...
   *
   * <p>The provided observer must implement the CardReaderObserverSpi interface
   * to be able to receive the events produced by this reader (card insertion,
   * removal, etc.)
   *
   ...
   *
   * <p>The observer will no longer receive any of the events produced by this
   * reader.
   *
  ...
  virtual void removeObserver(
      const std::shared_ptr<CardReaderObserverSpi> observer) = 0;
  ...
  /**
   * Starts the card detection. Once activated, the application can be notified
   * of the arrival of a card.
   *
   * <p>The DetectionMode indicates the action to be followed after processing
   * the card.
   *
   ...
   *
   * <p>This method notifies the observation process that the processing of the
   * card has been completed in order to ensure that the card monitoring cycle
   * runs properly.<br> It is <b>mandatory</b> to invoke it when the physical
   * communication channel with the card could not be closed. <br> This method
   * will do nothing if the channel has already been closed.<br> The channel
   * closing is nominally managed during the last transmission with the card.
   * However, there are cases where exchanges with the card are interrupted
   * (e.g. by an exception), in which case it is necessary to explicitly close
   * the channel using this method.
   *
   * @since 1.0.0
   */
